...
from autotrader.feeds import feeds_mt5, features
...
def test_get_bars_returns_utc(monkeypatch):
    ...
    spread_points = feed.get_spread_points("XAUUSD")
    assert spread_points == pytest.approx(10.0)


def test_build_context_uses_spread_points(monkeypatch):
    dummy = DummyMT5()
    monkeypatch.setattr(feeds_mt5, "mt5", dummy)
    feed = MT5Feed(
        BrokerConfig(
    ...
    feed.connect()
    df = feed.get_bars("XAUUSD", "M5", 100)
